...
<>{props.APIText = () => (props.doc) == "Rates" ? "the ShipperHQ Rates API without having to write any code." : (props.doc) == "Labels" ? "the ShipperHQ Labels API without having to write any code." : (props.doc) == "Insights" ? "the ShipperHQ Advanced Shipment Details API without having to write any code." : "different ShipperHQ APIs without having to write any code. You'll need the Endpoint URL for the API you're looking to experiment with as well as the authorization credentials for that API."}</>
